...
## 36.44 2026-09-12 后续授权：合并并推送主分支

用户在开发分支交付后明确要求“合并推送到主分支”。当前以远端 `main` 为基点合入 `improve-inference-reliability`，保留双方历史并同步安装说明；此前只推送开发分支的限制是前一轮范围记录。本次合并不改变冻结代码、异常暂缓决定或 S18 / W1b-5b / Release No-go。详见[项目状态](PROJECT_STATUS.zh-CN.md)。
